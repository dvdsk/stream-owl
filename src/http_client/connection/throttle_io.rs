...
impl<B> ThrottlableIo<B> {
    ...
    fn remove_lim(self: &mut Pin<&mut Self>) -> Result<(), std::io::Error> {
        self.set_lim(BandwidthLimit::practically_infinite())?;
        let this = self.as_mut().project();
    ...
    #[instrument(level = "debug", skip(self))]
    fn set_lim(self: &mut Pin<&mut Self>, limit: BandwidthLimit) -> Result<(), std::io::Error> {
        let this = self.as_mut().project();
...
impl<B: BandwidthCallback> hyper::rt::Read for ThrottlableIo<B> {
    fn poll_read(
        ...
        self.bandwidth_monitor.update(n_read);
